    ...
    if( idExists('#typeahead-tag-container') ) {
      ...
      var addCloseButtonListener = function() {
        ...
        tagCloseButtons.on('click', function(e, data) {
          var tagInputId = this.parentElement.dataset.tagInputId;
          this.parentElement.remove();
          var tagName    = this.parentElement.innerText;
      ...
      getAllPersistedTags($tagsURL, function(persistedTags) {
        ...
        var createTagsSuggestion = function() {
          var result = [];
        ...
        var tagsSuggestion = createTagsSuggestion();

        var appendTag = function(tagName) {
          if(tagIsNotAdded(tagName)) {
            var tagIndex = findIndex(tagName);
            var tag = persistedTags[tagIndex];
            var tagElement = "<div class='tag' data-tag-input-id='" + newTagCounter + "' id='tag-div-" + newTagCounter + "'>" + tagName + "<div class='tag-close-btn'>x</div></div>";
        ...
        var removeExistingTag = function(tagName) {
          var tagIndex = findIndex(tagName);
          if(tagIndex !== undefined) {
