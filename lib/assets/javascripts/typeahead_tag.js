    ...
    if( idExists('#typeahead-tag-container') ) {
      ...
      getAllPersistedTags($tagsURL, function(persistedTags) {
        ...
        var removeTag = function(tagName, tagInputId) {
          $('#' + tagInputId).remove();
          $currentModelTags.splice($currentModelTags.indexOf(tagName), 1);

          var persistedTag = findIndexInPersistedTags(tagName) === undefined ? false : true;

          if(persistedTag) {
            tagsSuggestion.push(tagName);
          }
          $typeaheadInput.focus();
          console.log($currentModelTags);
        };

        var createTagDiv = function(tagName) {
          var tagInputId = 'blog_tag_' + newTagCounter;
          var tagDiv = document.createElement('div');
          tagDiv.className  = 'tag';
          tagDiv.setAttribute('data-tag-input-id', tagInputId);
          tagDiv.setAttribute('data-tag-name', tagName);
          tagDiv.innerText = tagName;

          var tagCloseButton = document.createElement('div');
          tagCloseButton.className = 'tag-close-btn';
          tagCloseButton.innerText = 'x';

          tagDiv.appendChild(tagCloseButton);

          $(tagCloseButton).on('click', function() {
        ...
        var appendTag = function(tagName) {
          if(tagIsNotAdded(tagName)) {
            var tagIndex = findIndexInPersistedTags(tagName);

            var tagDiv = createTagDiv(tagName);
            $tagsWrapper.append(tagDiv);
        ...
        var findIndexInPersistedTags = function(tagName) {
          var result;
          $.each(persistedTags, function(i, tag) {
            if(tagName == tag.name) {
              result = i;
